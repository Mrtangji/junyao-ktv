# 改版骏耀K歌（junyao-ktv）
主要增加麦动曲库,LXMUSIC歌曲批量下载等功能

飞牛NAS DOCKER 搜索itangji/junyao-ktv
...
安卓客户端可以直接扫描设备本地 / 外接 U 盘里的音频来播放，**不占用服务器存储**：

- **扫描**：设置 →「扫描本机文件夹」，选一个目录（SAF 授权），会递归扫描其中的音频（mp3 / flac / m4a / aac / wav / ogg / opus）
- **播放**：经本机 `127.0.0.1` 流服务播放，支持拖动进度；App 端评分也能用（原生采集，不依赖 HTTPS）
- **授权会持久化**：扫描时申请的是持久授权，App 重启后仍可继续读取
- 若提示「文件夹授权已失效」（系统清理了授权、换了存储设备等），到 设置 →**「重新授权本机曲库」**点一下即可恢复：授权还在就直接按原文件夹重扫，丢了才重新拉文件夹选择器
- 浏览器端也能导入本机音频（存在 IndexedDB），但**必须 HTTPS** 才能评分
...
进入「管理后台 → 曲库管理」，将挂载进容器的目录逐个启用为曲库来源，保存后点一下「🔄 扫描曲库」完成入库。扫描已改为**手动触发**：启动容器时不再自动全量扫描（大曲库开机就逐文件 `ffprobe` 会把 CPU 吃满），平时往目录里丢新文件后手动扫一次即可。
...
在线下载同样支持无损：管理后台「歌手批量下载」的下载格式**默认即为「无损 FLAC」**（可改选 MP3/MV；电视端设置里的「点唱榜下载格式」会在 MP3 → FLAC → MV 之间循环），会先向音源请求 flac 音质，拿到无损就原样保存为 `.flac`（不转码），音源确实没有无损时自动回落 320K MP3。

### 6. 确认部署的是不是最新版

镜像 tag 固定是 `latest`，光看 tag 分不出是哪一次构建，所以服务端把自己的 **commit 短 sha + 构建时间**暴露出来（由 CI 在构建时烘进镜像）：

- **管理后台**（`/admin`）头部有版本徽标，登录页底部也有；点一下看构建详情，可一键跳到 GitHub 上对应的那次提交。
- **电视端 / 安卓端**：设置 → **服务端版本**（点击看详情），旁边还有 **App 版本**。
- **命令行**：`curl http://局域网IP:8080/api/version`，返回 `version`、`sha`、`buildTimeLocal`、`commitUrl` 等字段；`/api/diag` 里也带一份。
- **日志**：`docker logs <容器名> | grep VERSION`，启动第一屏就会打印。

拿这里的 **提交 sha** 和 GitHub 上最新提交对比：一致就是最新版。不一致一般是镜像还没拉下来或容器没重建：

```bash
docker compose pull && docker compose up -d     # 拉新镜像并重建容器
curl http://局域网IP:8080/api/version            # 再看一次 sha 是否已更新
```
