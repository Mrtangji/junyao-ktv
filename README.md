...
浏览器和安卓端的麦克风（getUserMedia）只在**安全上下文**下开放，因此评分必须走 HTTPS：

- 浏览器请打开 `https://局域网IP:8443/tv`（自签证书会报警告，点「高级 → 继续访问」即可）
- compose 里务必保留 `8443:8443` 映射：只映射 8080 的话，8443 在容器外不可达，**网页端和安卓端的评分都用不了**
- 安卓客户端会自动优先连 8443（自签证书在 App 内直接信任），连不上才回落 HTTP——回落时评分不可用，界面会有明确提示
- 电视盒子/老机型若提示「当前播放内核不支持麦克风采集」，请在系统里更新 **Android System WebView**
