// 回归测试：TV 播放时长配置（server/tvsettings.js）——默认值 / 持久化 / 夹取 / 脏数据回落。
//
// 跑法（仓库根目录）：
...
{
  const s = tvsettings.set({ ctlHideMs: 4000 });
  ok('set 返回新 ctlHideMs', s.ctlHideMs === 4000, String(s.ctlHideMs));
  const g = tvsettings.get();
  ok('get 读回持久化值', g.ctlHideMs === 4000, JSON.stringify(g));
}

console.log('=== C. 越界/非法值夹取 ===');
{
  tvsettings.set({ ctlHideMs: 100 }); // < MIN 500
  ok('低于下限 → 夹到 500', tvsettings.get().ctlHideMs === 500, String(tvsettings.get().ctlHideMs));
  tvsettings.set({ ctlHideMs: 999999 }); // > MAX 60000
  ok('高于上限 → 夹到 60000', tvsettings.get().ctlHideMs === 60000, String(tvsettings.get().ctlHideMs));
  tvsettings.set({ ctlHideMs: 'abc' }); // 非数值 → 默认
  ok('非数值 → 回落默认 3000', tvsettings.get().ctlHideMs === 3000, String(tvsettings.get().ctlHideMs));
}

console.log('=== D. 库里旧脏数据（0 秒）→ get 回落默认 ===');
{
  db.prepare("INSERT INTO settings(key,value) VALUES('tv_ctl_hide_ms','0') ON CONFLICT(key) DO UPDATE SET value='0'").run();
  ok('库值 0（非法）→ get 回落默认 3000', tvsettings.get().ctlHideMs === 3000, String(tvsettings.get().ctlHideMs));
}
