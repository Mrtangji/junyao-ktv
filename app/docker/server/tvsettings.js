...
//
// 生效优先级（TV 端 index.html 里实现）：URL 参数 ?ctlMs= > 服务端这份配置 > 默认值。
// URL 参数优先级最高，是为了在某个盒子/某台电视上单独调试而不用改全局。
//
// 全屏控制栏这类计时器上限设 60s：误填成毫秒以外的数（比如把「秒」当「毫秒」
// 填了 5）也不会离谱地一直不隐藏；下限 500ms，避免填 0 变成「闪一下就没」。
const db = require('./db');

const DEFAULTS = { ctlHideMs: 3000 };
const KEYS = { ctlHideMs: 'tv_ctl_hide_ms' };
const MIN_MS = 500;
const MAX_MS = 60000;
